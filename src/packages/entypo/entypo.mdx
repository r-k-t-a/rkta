...
<Playground>
  {() => {
    ...
    const Item = (item, name) => (
      <Paper name="Test" style={{width: '150px', height: '150px', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', marginLeft: '80px', marginBottom: '15px'}} key={name}>
        {item({ size: '128px' })}
